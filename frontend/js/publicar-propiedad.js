// frontend/js/publicar-propiedad.js
// ── CAMBIO respecto a la versión anterior ──
// submitForm ahora hace DOS llamadas:
//   1. POST /api/propiedades  (JSON con los datos)    → igual que antes
//   2. POST /api/imagenes/:id (FormData con archivos) → NUEVO, solo si hay fotos

let currentStep = 1;
const totalSteps = 6;
const counters = { habitaciones: 3, banos: 2, parqueos: 1 };
let uploadedPhotos = [];   // base64 para preview
let uploadedFiles  = [];   // File objects reales para enviar al servidor
let coverIndex = 0;
...
// ── CAMBIO: ahora guardamos tanto el base64 (preview) como el File real ──
function processFiles(files) {
  Array.from(files).forEach(file => {
    if(uploadedPhotos.length >= 20) return;
    // Guardar el File real para enviarlo al servidor
    uploadedFiles.push(file);
    // Guardar base64 solo para el preview visual
...
function deletePhoto(i) {
  ...
  uploadedFiles.splice(i, 1);  // ← también borramos el File real
...
// ── CAMBIO PRINCIPAL: submitForm ahora sube imágenes en segundo paso ──
async function submitForm() {
  ...
  try {
    // ── PASO 1: Crear la propiedad con los datos de texto ──
    ...
    const id_propiedad = respuesta.id_propiedad;

    // ── PASO 2: Subir imágenes si el usuario agregó alguna ──
    if (uploadedFiles.length > 0 && id_propiedad) {
      if (submitBtn) submitBtn.innerHTML = `Subiendo ${uploadedFiles.length} foto(s)...`;

      // Reordenar: la portada va primero
      const filesOrdenados = [...uploadedFiles];
      if (coverIndex !== 0) {
        const portada = filesOrdenados.splice(coverIndex, 1)[0];
        filesOrdenados.unshift(portada);
      }
    ...
    // ── PASO 3: Mostrar pantalla de éxito ──
    showSuccessScreen();

    if (id_propiedad) {
      sessionStorage.setItem('ultima_propiedad_publicada', id_propiedad);
    }
...
function resetForm() {
  currentStep = 1;
  uploadedPhotos = [];
  uploadedFiles  = [];  // ← limpiar también los File objects
  coverIndex = 0;
